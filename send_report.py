...
<div class="filter-panel">
  <div class="filters-grid">
    <div class="filter-group">
      <div class="filter-label"><span class="dot" style="background:#68d391"></span>均線排列</div>
      <select class="f-select" id="f-ma" onchange="render()">
        <option value="">全部</option>
        <option value="四線多排">四線多排（5/10/20/60）</option>
        <option value="有趨勢">有趨勢訊號</option>
      </select>
    </div>
    <div class="filter-group">
      <div class="filter-label"><span class="dot" style="background:#f6c650"></span>進場策略</div>
    ...
    <div class="filter-group">
      <div class="filter-label"><span class="dot" style="background:#fc8181"></span>最低爆量倍數</div>
      <input class="f-input" type="number" id="f-vol" value="1" min="1" step="0.1" onchange="render()">
    </div>
  </div>
  <div class="toggle-row">
    <span class="toggle-label">產業：</span>
    <button class="toggle-btn on" data-type="industry" data-val="" onclick="setFilter('industry','',this)">全部</button>
    {ind_btns}
  </div>
  <div class="toggle-row">
    <span class="toggle-label">市值：</span>
    <button class="toggle-btn on" data-type="cap" data-val="" onclick="setFilter('cap','',this)">全部</button>
    {cap_btns}
  </div>
  <div class="toggle-row">
    <span class="toggle-label">訊號：</span>
...
<script>
const ALL = {data_json};
let sKey='score', sDir=-1, actInd='', actCap='', actSigs=new Set();
...
function getFiltered(){{
  ...
  return ALL.filter(s=>{{
    ...
    if(actInd && s.industry!==actInd) return false;
    if(actCap && s.cap_size!==actCap) return false;
...
function setFilter(type,val,btn){{
  if(type==='industry')actInd=val;
  else actCap=val;
  document.querySelectorAll(`[data-type="${{type}}"]`).forEach(b=>b.classList.remove('on'));
  btn.classList.add('on');
  render();
}}

function toggleFilter(btn){{
  setFilter(btn.dataset.type,btn.dataset.val,btn);
}}
