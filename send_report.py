...
.breakout-section{{display:none;background:rgba(240,180,41,.05);border:1px solid rgba(240,180,41,.2);border-radius:4px;padding:16px;margin:16px 0}}
.breakout-section.show{{display:block}}
...
<div class="filter-panel">
  <div class="filters-grid">
    <div>
      ...
      <select class="f-select" id="f-entry" onchange="showBreakoutParams(); render()">
  ...
  <div id="breakoutSection" class="breakout-section">
...
<div class="table-wrap">
  <table>
    <thead><tr style="background:var(--surface2)">
      <th>代號</th><th>名稱</th><th>昨日收盤</th><th>昨日量</th><th>今日收盤</th><th>今日量</th>
      <th>5日均價</th><th>漲跌%</th><th>上影線%</th><th>量比(vs5日)</th><th>量比(vsYesterday)</th>
      <th>評分</th><th>訊號</th><th>產業</th><th>市值</th><th>策略</th>
    </tr></thead>
...
function showBreakoutParams() {{
  const entry = document.getElementById('f-entry').value;
  const section = document.getElementById('breakoutSection');
...
<div style="max-width:680px;margin:20px auto;background:#fff;border-radius:10px;overflow:hidden">
  ...
  <div style="padding:20px">
    <table style="width:100%;border-collapse:collapse;font-size:12px">
      ...
      <tbody>
        {''.join(f'<tr style="border-bottom:1px solid #e5e7eb"><td style="padding:10px"><a href="https://www.wantgoo.com/stock/{s["code"]}/technical-chart" style="color:#2563eb;text-decoration:none">{s["code"]}</a></td><td style="padding:10px">{s["name"]}</td><td style="padding:10px;text-align:right">{s["price"]}</td><td style="padding:10px;text-align:right;color:{"#fc8181" if s["chg"]>=0 else "#68d391"}">{("+"+str(s["chg"]) if s["chg"]>=0 else str(s["chg"]))+"%"}</td><td style="padding:10px;text-align:right;font-weight:700">{s["score"]}</td></tr>' for s in sorted(candidates, key=lambda x: x["score"], reverse=True)[:50])}
      </tbody>
    </table>
  </div>
